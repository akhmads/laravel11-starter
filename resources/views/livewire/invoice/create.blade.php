...
new class extends Component {
    ...
    {
        $data = $this->validate([
            'number' => 'required|max:50|unique:invoices,number',
            'date' => 'required|date',
            'note' => 'nullable',
            'items' => 'array|min:1',
        ]);
        ...
        $this->items->each(function ($item, $key) use ($invoice) {
        ...
        $this->items = collect([]);
        $this->success('Invoice has been created.', redirectTo: '/invoice');
    }
...
<div>
    <x-header title="Create Invoice" separator>
        <x-slot:actions>
